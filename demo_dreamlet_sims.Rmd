...
run_method = function(pb, formula, method){
  ...
  if( method == "dreamlet"){
    res.time = system.time({
      res.proc = processAssays( pb, ~ 1, BPPARAM=SnowParam(12), quiet=TRUE, min.samples=6) 
...
```{r run.sims}
n_batches = c(3, 6, 12, 18, 25, 36)[1:2]
methods = c('dreamlet', "DESeq2")

resTabl = lapply(n_batches, function(nbatch){

  idx = pbObj.Donor$BatchID %in% levels(pbObj.Donor$BatchID)[1:nbatch]

  resTabl = lapply(methods, function(method){
    res1 = run_method( pbObj.Donor[1:100,idx], ~ Dxsim, method)
    res2 = run_method( pbObj.Donor[1:100,idx], ~ Dxsim + BatchID, method)
    if( method == "dreamlet"){      
      res3 = run_method( pbObj.Donor[1:100,idx], ~ Dxsim + (1|BatchID), method)
    }else{
      res3 = data.frame()
    }
