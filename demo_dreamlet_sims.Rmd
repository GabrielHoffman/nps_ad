...
resTabl = lapply(n_donors, function(n){
...
# # fixed
# resTabl2 = lapply(n_donors[1:3], function(n){
#   resTabl = lapply(methods, function(method){
#     message(n, ' ', method)
#     run_method( pb[,seq(1,n)], ~ Dxsim + BatchID, method)
#   })
#   ids = sapply(resTabl, colnames)
#   tab = table(ids)
...
#   do.call(rbind, lapply(resTabl, function(x) x[,names(tab)[tab == length(resTabl)]]))
# })
# resTabl2 = do.call(rbind, resTabl2)
# resTabl2$method = paste(resTabl2$, "- fixed" )
# resTabl = rbind(resTabl, resTabl2)

# Random
resTabl3 = lapply(n_donors, function(n){
  resTabl = lapply('dreamlet', function(method){
    message(n, ' ', method)
    run_method( pb[,seq(1,n)], ~ Dxsim + (1|BatchID), method)
  })
  ids = sapply(resTabl, colnames)
  tab = table(ids)
...
resTabl3 = do.call(rbind, resTabl3)
resTabl3$method = paste(resTabl3$, "- random" )

resTabl = rbind(resTabl, resTabl3)
```
