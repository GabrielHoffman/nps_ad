...
for( batch in levels(pb$BatchID)){
  ...
  if(sim.method == "balanced"){
    ...
    ids.select = ids[which(sample(0:1, length(ids), replace=TRUE)==1)]
  }
...
hist(tab[,1] / rowSums(tab), xlab="AD fraction")
```

#### variancePartition
```{r vp}
res.proc = processAssays( pbObj.Donor, ~ 1)
...
vp.lst2 = fitVarPart( res.proc, ~ Dxsim, BPPARAM=SnowParam(12))
...
run_analysis = function(n_batches, methods){
  ...
  resTabl = mclapply(n_batches, function(nbatch){
    ...
    resTabl = lapply(methods, function(method){
      ...
      res1 = run_method( pbObj.Donor[,idx], ~ Dxsim, method)
      res2 = run_method( pbObj.Donor[,idx], ~ Dxsim + BatchID, method)
      # if( method %in% c("dreamlet", "glmer")){      
      #   res3 = run_method( pbObj.Donor[,idx], ~ Dxsim + (1|BatchID), method)
      # }else{
        res3 = data.frame()
      # }
      gc()
