...
run_analysis = function(n_batches, methods){
  ...
  resTabl = mclapply(n_batches, function(nbatch){
    ...
    resTabl = lapply(methods, function(method){
      ...
      res1 = run_method( pbObj.Donor[1:2000,idx], ~ Dxsim + cohort, method)
      res2 = run_method( pbObj.Donor[1:2000,idx], ~ Dxsim + cohort + BatchID, method)
      if( method %in% c("dreamlet", "glmer")){      
        res3 = run_method( pbObj.Donor[1:2000,idx], ~ Dxsim + cohort + (1|BatchID), method)
      }else{
        res3 = data.frame()
      }
      gc()
...
ggplot(df, aes(n, time.elapsed/60, color=method, formula)) + 
    geom_line() + 
    geom_point() + 
    theme_classic() +
...
ggplot(df, aes(n, time.user/60, color=method)) + 
    geom_line() + 
    geom_point() + 
    theme_classic() +
    theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + 
...
ggplot(df, aes(factor(n), FPR, fill=method)) + 
    ...
    facet_wrap(~formula, scales = "free_x", space = "free_x")
```
