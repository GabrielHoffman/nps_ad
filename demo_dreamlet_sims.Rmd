...
run_analysis = function(n_batches, methods){
  ...
  resTabl = mclapply(n_batches, function(nbatch){
    ...
    resTabl = lapply(methods, function(method){
      ...
      res1 = run_method( pbObj.Donor[1:200,idx], ~ Dxsim, method)
      res2 = run_method( pbObj.Donor[1:200,idx], ~ Dxsim + BatchID, method)
      # if( method %in% c("dreamlet", "glmer")){      
      #   res3 = run_method( pbObj.Donor[1:2000,idx], ~ Dxsim + (1|BatchID), method)
      # }else{
        res3 = data.frame()
      # }
      gc()
