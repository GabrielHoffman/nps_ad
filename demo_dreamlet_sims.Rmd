...
run_method = function(pb, formula, method){
  ...
  if( method == "dreamlet"){
    ...
    res = lapply(assayNames(res.proc), function(CT){
        ...
        counts = assay(pb.sub, CT)

        res.glmer = lapply(1:nrow(pb.sub), function(i){
          y = counts[i,]

          form = as.formula(paste('y ~', as.character(formula)[2] ,'+ (1|SubID)'))

          fit = glmer(formula, data=colData(pb.sub))
          coef(summary(fit))
