...
```{r vp.examine, fig.height=8, fig.width=6}
CT = "Astrocyte"
...
fig_pct = plotPercentBars( data.frame(df[i,-c(1,2)])) + ggtitle(CT)
...
figList = lapply( df$gene[i], function(geneName){
  ...
  plotStratifyBy(df[1:100,], 'group_id', geneName, colorBy=NULL, main=geneName) + theme(aspect.ratio=1)
})

fig_pct + theme(aspect.ratio=.2)
