...
df_sub = df_res[(df_res$geneID=="1") &(df_res$dx %in% c('ad', 'Control')),]
df_sub = droplevels(df_sub)
fit = lmer( abs(zScore) ~ scale(rawcounts) + (1|celltype) + (1|SubID) + dx, data = )
...
figList = lapply(rownames(ods), function(x){
  plotExpObsCounts(ods, x) + theme_classic() 
})
