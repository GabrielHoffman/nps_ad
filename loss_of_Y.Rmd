...
df_res = res %>% 
  ...
  mutate(Channel = gsub('-.*$','', sampleID),
         se = l2fc / zScore) %>% 
  merge(with(colData(sceCombine), data.frame(sampleID = colnames(sceCombine), celltype)), by="sampleID")  
...
source("~/workspace/repos/nps_ad/helper/plotExpObsCounts.R")
