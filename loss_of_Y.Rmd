...
df_uniq = with(colData(sceCombine), unique(data.frame(Channel, SubID)))
...
fit = glmer(nAbb ~ offset(log(nObs)) + (1|SubID) + (1|geneID) + (1|celltype) + scale(log(totalCounts)), df_counts_aggr, family="poisson")
...
fit = lmer( abs(zScore) ~ scale(rawcounts) + (1|celltype) + (1|Channel) + (1|SubID) + as.numeric(dx), data = df_sub)

calcVarPart(fit)

summary(fit)

lapply( levels(df_res$geneID), function(chrom){
  ...
  rbind(data.frame(Direction="loss", t(coef(summary(fit1))['as.numeric(dx)',])),
        data.frame(Direction="gain", t(coef(summary(fit2))['as.numeric(dx)',])))
