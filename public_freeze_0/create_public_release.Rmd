...
if( file.exists(datafile) ){
  ...
  # Public freeze 0
  h5ad_file = "/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/h5ad/221210_NPS-AD_public0_pass3_anno.h5ad"
...
# 
# and no Sex_chr_aneuploidy 
sce = sce[,(!is.na(sce$Sex_chr_aneuploidy)) & !sce$Sex_chr_aneuploidy ]

# Remove columns for public release
keep = c('SubID', 'Channel', 'Brain_bank', 'round_num', "poolID", 'Sex', 'Age', 'AD', 'Ethnicity', 'pH', 'PMI' , "class", "subclass", "subtype")
colData(sce) = droplevels(colData(sce)[,keep])
```


rowData(sce)[rowData(sce)$gene_chrom=="X",]
...
# Create a data.frame of UTY and LAS1L
geneID = c("Sex", "LAS1L", "UTY")
dfSub = df[,geneID] 
dfSub$Sex = factor(dfSub$Sex, c("Male", "Female"))

ggplot(dfSub, aes(LAS1L, UTY, color=Sex)) +
    geom_point() +
    theme_classic() +
...
fit = glm(Sex ~ LAS1L + UTY, dfSub, family="binomial")
...
ggplot(dfSub[drop,], aes(LAS1L, UTY, color=abs(score))) +
    geom_point() +
    theme_classic() +
    theme(aspect.ratio=1) +
    scale_color_gradient(low="black", high="orange", limits=c(0, 1))
...
outprefix = "/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Dec_9_2022"
...
echo $(ls /sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Sept24_2022_chunk*.h5ad) | tr ' ' '\n' > list.txt

cd /hpc/users/hoffmg01/.cache/R/basilisk/1.8.0/0/pkgs/
OUTFILE=/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Sept24_2022.h5ad 
