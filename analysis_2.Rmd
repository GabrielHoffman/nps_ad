...
plotVolcano( res.dl, coef = coefs[1] ) + ggtitle(coefs[1])
```

```{r volcanos.2, fig.width=7, fig.height=12}
plotVolcano( res.dl, coef = coefs[2] ) + ggtitle(coefs[2])
```

```{r volcanos.3, fig.width=7, fig.height=12}
plotVolcano( res.dl, coef = coefs[3] ) + ggtitle(coefs[3])
```
...
plotZenithResults(res.gsa[[1]], 5, 2) + ggtitle(names(res.gsa)[1])
```

```{r zenith.heatmap.2, fig.width=7, fig.height=12}
plotZenithResults(res.gsa[[2]], 5, 2) + ggtitle(names(res.gsa)[3])
```

```{r zenith.heatmap.3, fig.width=7, fig.height=12}
plotZenithResults(res.gsa[[3]], 5, 2) + ggtitle(names(res.gsa)[3])
```
...
res_ctc = cellTypeCompositionTest( pbObj, form, "Dx2BP", "dream")
