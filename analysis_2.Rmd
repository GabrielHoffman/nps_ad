...
ggplot(df_melt, aes(Var2, value, fill=Var2)) + 
  ...
  geom_hline(yintercept=1/ncol(df), color="red", linetype="dashed")

# Barplot of 5 genes
genes = c(rownames(df)[1:5], 'ENSG00000151067')
...
df = unique(data.frame(Dx2 = pbObj$Dx2, Donor = pbObj$group_id))
...
res.proc = processAssays( pbObj, ~ (1|group_id) + (1|batch) + (1|round_num) + (1|HTO) + (1|cohort), 
  min.cells = 5,
  min.count = 10, 
...
```{r varPart}
form = ~ (1|group_id) + (1|batch) + (1|round_num) + (1|HTO) + (1|Dx2) + (1|Sex) + (1|cohort) + scale(Age) + scale(PMI) 
...
```{r within_btw_sub, fig.height=6, fig.width=12}
eval_within_across_donor( res.proc[7:9] )
```


```{r dreamlet.fit}
form = ~ Sex + (1|group_id) + (1|batch) + (1|round_num) + (1|HTO) + (1|cohort) + Dx2 + scale(Age) + scale(PMI)
...
resMatList = lapply( assayNames(fit.for.resid), function(CT){
  ...
  resMatCollapse = lapply( unique(info$group_id), function(grpid){
    idx = which(info$group_id == grpid)

    rowMeans(resMat[,idx,drop=FALSE])
    })
  resMatCollapse = do.call(cbind, resMatCollapse)
  colnames(resMatCollapse) = unique(info$group_id)
...
# Add dream here
# deal with NA's here 
form = ~ (1|group_id) + (1|batch) + (1|round_num) + (1|HTO) + (1|Sex) + (1|Dx2) + (1|cohort) + scale(Age) + scale(PMI)
...
colnames(df_vp) = gsub("group_id", "Donor", colnames(df_vp))
...
form = ~ (1|group_id) + (1|batch) + (1|round_num) + (1|HTO) + (1|Sex) + (1|cohort) + Dx2 + scale(Age) + scale(PMI)
