...
system("git pull"); rmarkdown::render("analysis_freeze1.Rmd");
...
res.vp = fitVarPart(res.proc, form, BPPARAM = SnowParam(6, progressbar=TRUE), assay='Glutamatergic neuron')
```
...
```{r varPart}
form = ~ (1|SubID) + (1|batch) + (1|round_num) + (1|HTO) + (1|Sex) + (1|Institution) + scale(Age)
...
```{r dreamlet.fit.many}
form = ~ Sex + (1|SubID) + (1|batch) + (1|round_num) + (1|HTO) + (1|Institution) + scale(Age)

test.traits = c('AD', 'MCI', 'Dementia', 'PD', 'PD_uncertain_plus_encephalitic', 'DLBD', 'FTD','ALS')
...
fitList = lapply( test.traits, function(x){
  ...
  dreamlet( res.proc, formula, BPPARAM = SnowParam(12) )
  })
...
res.gsa = lapply( names(fitList), function(x){

  zenith_gsa(fitList[[x]], coefs=x, go.gs)
})
names(res.gsa) = names(fitList)
```
...
form = ~ Sex + (1|SubID) + (1|batch) + scale(Age) # + Institution

test.traits = colnames(colData(res.proc))[c(29:66, 74:111)]
test.traits = c(test.traits, 'CERAD', "BRAAK_AD", "BRAAK_PD" ,  "CDRScore",  "Plq_Mn", "ApoE_gt")

values = sort(apply(colData(res.proc)[,test.traits], 2, var, na.rm=TRUE))

test.traits = names(values)[values > 0.1]
...
fitList = lapply( test.traits, function(x){
  ...
  fit = dreamlet( res.proc, formula, BPPARAM = SnowParam(24), quiet=TRUE )
  })
names(fitList) = test.traits

file = "/sc/arion/projects/CommonMind/hoffman/NPS-AD/work/nps_ad/freeze_v1/fitList.test.RDS"
saveRDS(fitList, file=file)
```
