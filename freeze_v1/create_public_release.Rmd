...
bsub -Is -q premium -R span[hosts=1] -R rusage[mem=48000] -W 24:00 -P acc_CommonMind -n 12 bash
...
tab = with(colData(sceCombine), unique(data.frame(SubID, AD, Sex)))
table(sceCombine$AD)
table(sceCombine$Sex)
...
outprefix = "/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Sept16_22"

res = lapply(names(chunks), function(id){
  message(id)
  outfile = paste0(outprefix, '_chunk', id, ".h5ad")
  sceSub = sce[,chunks[[id]]]
  writeH5AD(sceSub, outfile, compression="lzf")
  })
```
...
echo $(ls /sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Sept16_22_chunk*.h5ad) | tr ' ' '\n' > list.txt

OUTFILE=/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Sept16_22.h5ad 
python $SRC -i list.txt -o $OUTFILE
```
