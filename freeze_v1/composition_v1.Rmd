...
df_cellCounts = infoFull %>% 
  group_by(all_of(c('Channel', CT_FOCUS))) %>%
  count() %>%
...
info = infoFull %>%
  group_by(Channel) %>%
  select(Channel, SubID, round_num, batch, prep, dx, Institution, Age, Ethnicity, )  %>%
  distinct  %>%
  column_to_rownames("Channel")
...
figLst = lapply(names(tab), function(lvl){
  ...
  fig = plotCellComposition(df_cellCounts[idx,]) + ggtitle(lvl) 
...
fractions = t(apply(df_cellCounts[i,], 1, function(x){
  x = x+1
  x / sum(x)
}))
