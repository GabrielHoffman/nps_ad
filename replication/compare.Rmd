...
system("git pull"); rmarkdown::render("compare.Rmd");
...
plotTiles = function(df1, df2, xlab, ylab){
  ...
  fig = df %>%
    ...
    ggplot(aes(CT1, CT2, fill=pmax(rho, 0))) +
      geom_tile() +
      geom_text(aes(label=ifelse(isMatch, "*", "")), size=10, hjust="middle", vjust=.9) +
      theme_classic() +
...
df2 = res.seaad %>% 
      ...
      mutate(assay = factor(assay, cellTypeOrder$SEA_AD_subclass)
