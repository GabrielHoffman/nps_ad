...
res_pred = lapply(1:nrow(grid), function(i){
  ...
  if(grid$geneID[i] == "Y"){
    # form1 = dx ~ zScore + I(zScore^2) + Age 
    form1 = dx ~ zScore.loss + zScore.gain + Age
    form2 = dx ~ RankNorm(ad_pred_cells) + Age 
  }else{
    # form1 = dx ~ zScore + I(zScore^2) + Age + Sex
    form1 = dx ~ zScore.loss + zScore.gain + Age + Sex
...
ggplot(res_pred, aes(chrom, celltype, fill=z.value)) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio) + scale_fill_gradient2(low = "blue", mid="white", high="red", limits=c(-zlim, zlim))
```
