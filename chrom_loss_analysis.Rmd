...
ggplot(res_lm[res_lm$coef=="dxAD",], aes(geneID, celltype, fill=sign(z.value)*log10(abs(z.value)))) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio, plot.title = element_text(hjust = 0.5)) + scale_fill_gradient2(name=expression(sign~log[10]~z), low = "blue", mid="white", high="red", limits=c(-zmax, zmax))  + facet_wrap(~ Direction, ncol=1) + ggtitle("dxAD")

ggplot(res_lm[res_lm$coef=="scale(Age)",], aes(geneID, celltype, fill=sign(z.value)*log10(abs(z.value)))) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio, plot.title = element_text(hjust = 0.5)) + scale_fill_gradient2(name=expression(sign~log[10]~z), low = "blue", mid="white", high="red", limits=c(-zmax, zmax))  + facet_wrap(~ Direction, ncol=1) + ggtitle("Age")
```
...
ggplot(res_lmz[res_lmz$coef=="dxAD",], aes(geneID, celltype, fill=t.value)) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio, plot.title = element_text(hjust = 0.5)) + scale_fill_gradient2(low = "blue", mid="white", high="red", limits=c(-zlim, zlim)) + facet_wrap(~ Direction, ncol=1) + ggtitle("dxAd")

ggplot(res_lmz[res_lmz$coef=="scale(Age)",], aes(geneID, celltype, fill=t.value)) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio, plot.title = element_text(hjust = 0.5)) + scale_fill_gradient2(low = "blue", mid="white", high="red", limits=c(-zlim, zlim)) + facet_wrap(~ Direction, ncol=1) + ggtitle("Age")
```
...
ratio = with(res_pred, nlevels(celltype) / nlevels(chrom))
...
ggplot(res_pred, aes(chrom, celltype, fill=z.value)) + geom_tile() + theme_classic() + theme(aspect.ratio=ratio) + scale_fill_gradient2(low = "blue", mid="white", high="red", limits=c(-zlim, zlim)) + geom_text(aes(label=ifelse(fdr < 0.05, '*', '')))
```
...
data = df_res %>%
        ...
        select(geneID, celltype, SubID, Channel, zScore, zScore.loss, zScore.gain, Age, Sex, dx)  %>%
...
df = data %>%
      pivot_wider( names_from = c(celltype, geneID), values_from=c(zScore, zScore.loss, zScore.gain)) %>%
      droplevels
...
df_pred = lapply( keys, function(x){
  ...
  if( x == "Sex + Age + MA"){
    ...
    df_beta = cbind(df_beta, df_beta$coef %>% 
                strsplit("_") %>% 
                do.call(rbind, .))
    colnames(df_beta)[-c(1:3)] = c("score", "celltype", "chrom")

    df_beta$chrom = factor(df_beta$chrom, c(1:22, "X", "Y", "MT")) 

    ratio = with(df_beta, length(unique(celltype)) / nlevels(chrom))

    fig = ggplot(df_beta, aes(chrom, celltype, fill=value)) + 
              geom_tile() +
              theme_classic() +
  ...
  df_pred = lapply(levels(df$dx), function(status){
    ...
    y_pred_not = y_pred[df$dx[-training]!=status, status]
...
fig = ggplot(df_pred, aes(Set, AUPR, fill=Set)) + 
        facet_wrap(~ Class) +
...
Sex + Age predictor: 0.86
Sex + Age + chrom aneuploidy predictor: 0.88
...
fig = ggplot(data2[idx,], aes(dx, mu.z.loss)) + geom_violin() + geom_boxplot(width=0.1) + facet_wrap(~ geneID + celltype, ncol=4) + theme_classic() + theme(aspect.ratio=1)

ggsave("test.png", fig, height=12)


fig = ggplot(data2[idx,], aes(dx, abber.rate.down)) + geom_violin() + geom_boxplot(width=0.1) + facet_wrap(~ geneID + celltype, ncol=4) + theme_classic() + theme(aspect.ratio=1)

ggsave("test.png", fig, height=12)

fig = ggplot(data2[idx,], aes(dx, pi.loss)) + geom_violin() + geom_boxplot(width=0.1) + facet_wrap(~ geneID + celltype, ncol=4) + theme_classic() + theme(aspect.ratio=1)
