...
res.procList = lapply( pbList, function(pb){
  # Normalize and apply voom
  processAssays(pb, ~ (1|SubID) + (1|batch) + (1|HTO) + (1|cohort),  # + (1|round_num)
    min.cells = 5,
    min.count = 10, 
    BPPARAM = SnowParam(16, progressbar=TRUE))
})   
...
```{r varPart.2}
form = ~ (1|SubID) + (1|batch) + (1|HTO) + (1|Dx2) + (1|Sex) + (1|cohort) + scale(Age) + scale(PMI)  #  + (1|round_num)
...
```{r examine, fig.width=8, fig.height=20}
library(viridis)
...
```{r compare, fig.width=7, fig.height=12}
...
ggplot(df, aes_string(a,b)) + geom_point(aes(color=density), size=.02) + facet_wrap(~assay.y, ncol=3) + theme_classic() + theme(aspect.ratio=1) + geom_abline(color="grey50", linetype="dashed", width=.3)  + scale_color_viridis()
```
...
df %>%
  ...
  ggplot(aes(assay.y, Corr, fill=assay.y)) + 
    ...
    coord_flip() +
    ylab("Correlation in batch effect across freezes") +
    xlab("Cell type") +  
...
df_res = lapply(colnames(df)[5:14], function(key){
  res = df2 %>% 
    as_tibble %>% 
    group_by(assay.y) %>%  
...
fig = ggplot(df_res, aes(assay.y, key, fill=estimate)) + geom_tile() + geom_text(data=subset(df_res, fdr < 0.05), aes(assay.y, key,label='*')) + scale_fill_gradient2(low="blue", mid="white", high="red", limits=c(-1, 1))
