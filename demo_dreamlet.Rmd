...
f(res.dlList) to evaluate concordance
...
tabLst = lapply( coefs, function(key){
  tab = lapply( names(res.dlList), function(x){
    tab = topTable(res.dlList[[x]], coef=key, number=Inf)
    data.frame(Set = x, tab)
  })
...
df_res = lapply(colnames(df)[5:13], function(key){
...
df_res %>%
  ...
  group_by(assay.y) %>%
  ggplot(aes(assay.y, estimate, fill=assay.y)) + 
    geom_bar(stat="identity") +
    geom_errorbar(aes(ymin = pmax(0,estimate-1.96*se), ymax = estimate+1.96*se), width=0) +
    theme_classic() +
    theme(aspect.ratio=1, legend.position="none") +
    coord_flip() +
    ylab("Correlation in batch effect and GC content per gene") +
    xlab("Cell type") +  
    scale_fill_discrete(name = "Cell type") +
    scale_y_continuous(limits=c(0,.26), expand=c(0,0)) +
    geom_text(aes(y=0.23, label=ifelse(fdr < 0.05, "            #", '')))
```
