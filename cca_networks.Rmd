...
exclude = c('Endothelial', 'Unknown', 'OPC', 'Mural', "GABAergic","GABAergic PVALB+", "GABAergic SST+", "GABAergic VIP+")
keep = ! names(scoreList) %in% exclude

geneExpr = lapply( names(scoreList)[keep], function(CT){
  cat(CT)

  geneExpr = scoreList[[CT]]

  df = select(org.Hs.eg.db, keys = rownames(geneExpr), columns="CHR", keytype="SYMBOL")
...
pairwise_cca = function(exprList, verbose=TRUE){
  ...
  df = apply(idx_combn, 2, function(x){
    ...
    data.frame(CT1 = CT1, CT2 = CT2, N = nrow(X_sub), lambda.x = fit.cca$lambdas[1],
      lambda.y = fit.cca$lambdas[2], cramer.V = fit.cca$cramer.V, cor1 = fit.cca$cor[1], rho1 = fit.cca$rho.mod[1], ri.x=sum(fit.cca$x.ri), ri.y=sum(fit.cca$y.ri), n.x=ncol(X_sub), n.y=ncol(Y_sub)) 
