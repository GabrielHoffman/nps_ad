...
make_plot = function(coef1, coef2){
  ...
  # add CMC  
  df1 = df.cmc %>%
    select(ID, assay, estimate) %>%
    filter(assay == "MPP") %>%
    rename(estimate.MPP = estimate) %>%
    select(-assay) 
  ...
  df2 = df.cmc %>%
    select(ID, assay, estimate) %>%
    filter(assay == "HBCC") %>%
    rename(estimate.HBCC = estimate) %>%
    select(-assay)  %>%
    filter(ID %in% genes) %>%
    inner_join(df1 %>% filter(ID %in% genes), by="ID")

  cor(df2$estimate.MPP, df2$estimate.HBCC)
...
make_plot = function(coef1, coef2){
  ...
  # add CMC  
  df1 = df.cmc %>%
    select(ID, assay, p.value) %>%
    filter(assay == "MPP") %>%
    rename(p.MPP = p.value) %>%
    select(-assay) 
...
file = "/sc/arion/scratch/hoffmg01/CMC_Human_clinical_metadata.csv"
info2 = read_csv(file) %>%          
