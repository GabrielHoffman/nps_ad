...
ls jobs/* | grep crumblr | wc | parallel -P1 "bsub < {}"
...
<font size="5">__AnnoLevel:__ `r params$AnnoLevel`</font>

<font size="5">__SampleLevel:__ `r params$SampleLevel`</font>
...
pattern = paste0(toupper(params$dataset), "_2023-04-01.*_PB_", params$SampleLevel, "_", params$AnnoLevel, ".RDS")
file = dir(path, pattern=pattern, full.names=TRUE)

pb = readRDS( file )

covariates_base = c("Age", "(1|Sex)", "PMI") 
...
col = c(brewer.pal(ncol(vp.c)-1, "Set1"), "grey85")
fig.vp = plotPercentBars( sortCols(vp.c), col=col )

plotVarPart(sortCols(vp.c), label.angle=60, ncol=4, col=col ) + theme(aspect.ratio=1)
...
formula: `r paste0(as.character(form), collapse=' ')`

formula vp: `r paste0(as.character(form.vp), collapse=' ')`
...
ggsave(file="~/www/test.pdf", figList[[1]]$fig.combine, width=15, height=8)


ggsave(file="~/www/test.pdf", fig1, width=15, height=8)
