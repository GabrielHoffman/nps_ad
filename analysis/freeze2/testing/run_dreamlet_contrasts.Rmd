...
covariates_base = c("scale(Age)", "Sex", "scale(PMI)", "log(n_genes)", "percent_mito", "mito_genes", "ribo_genes", "mito_ribo")
...
# if less than 15 samples have values for the give trait
# remove 
s1 = sum(!is.na(colData(res.proc)[[ctr$variable]]))
s2 = sum(!is.na(colData(res.proc)[[ctr$name]]))
s = max(s1, s2)
if( s < 15 ){
  exclude = attr(terms(form), "term.labels")[-c(1:5)]
  exclude = paste("~ . -", paste(exclude, collapse=' - '))
  exclude = as.formula(exclude)

  form = update(form, exclude)
}
