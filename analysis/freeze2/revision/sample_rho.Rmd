...
ggplot(df, aes(hsq, geneExpr)) +
  geom_errorbar(aes(ymin = geneExpr - 1.96*geneExpr.se, ymax = geneExpr + 1.96*geneExpr.se, color=type), alpha = .2) +
  geom_errorbar(aes(xmin = hsq - 1.96*hsq.se, xmax = hsq + 1.96*hsq.se, color=type), alpha = .2) +
