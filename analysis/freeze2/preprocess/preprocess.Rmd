...
augment_data = function(pb){
  ...
  colData(pb) = cbind(colData(pb), df[i,-c(1:3)])

  # Compute PC's of technical variables
  # append then to colData
  X = scale(df[i,-c(1:3)])
  X = X[,apply(X, 2, function(a) all(!is.nan(a)))]
  dcmp = prcomp(X)
...
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
    ...
    # formula
    form = ~ Sex + scale(Age) + scale(PMI) + (1|mod_BRAAK_AD) + mod_Plq_Mn + (1|mod_CERAD) + (1|mod_CDRScore) + mod_Cognitive_Resilience + (1|MajorDisease) + (1|DementiaType) + (1|mod_cogdx) + log(n_genes) + TechPC1 + TechPC2 + TechPC3 
    ...
    # processAssays
    res.proc = processAssays( pb, form, BPPARAM = bpparam, weightsList = W.list, min.samples=20)
    flush(stderr())
