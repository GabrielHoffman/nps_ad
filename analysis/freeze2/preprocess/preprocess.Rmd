...
# put residuals in SCE
for(id in names(res.proc2)){
  res.proc2[[id]]$E = residList[[id]]
}
```

```{r pca.plot}
# PCA plot showing outliers
plotPCA(res.proc, maxOutlierZ=6)

# if n_slice = 1, for each sample report the smallest FDR
# across cell types.
# if n_slice = 2, for each sample report the 2nd smallest FDR
# across cell types.  This is stricter and excludes only samples 
# that are outliers in at least 2 celltypes
n_slice = switch(names(params$DATASET), 
            RUSH = 1,
            MSSM = 2,
            HBCC = 2,
            AGING = 1,
...
df_outlier = outlierByAssay(res.proc) %>%
...
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
    ...
    if( params$DATASET == "AGING" ){
      form = ~ Sex + scale(Age) + scale(PMI) + Source + log(n_genes) + percent_mito + mito_genes + mito_ribo + ribo_genes
    }
