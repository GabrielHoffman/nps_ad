...
# bsub -Is -q premium -R span[hosts=1] -R rusage[mem=50000] -W 128:00 -P acc_CommonMind -n 12 bash
...
run = function(x){
  ...
  create.folder(tmp.dir)
  setwd(tmp.dir)

  fold = "/sc/arion/projects/CommonMind/hoffman/NPS-AD/work/nps_ad/analysis/freeze2/preprocess"
  x %>% 
    walk(function(x) render(paste0(fold, "/preprocess.Rmd"),
...
```{r processAssays, eval=TRUE, message=TRUE}
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
