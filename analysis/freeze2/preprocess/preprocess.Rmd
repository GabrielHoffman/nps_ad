...
# bsub -Is -q premium -R span[hosts=1] -R rusage[mem=30000] -W 96:00 -P acc_CommonMind -n 12 bash
...
# March 26
h5ad_files = c(
RUSH = "/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/h5ad_final/RUSH_2023-03-26_22_43.h5ad", 
HBCC = "/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/h5ad_final/HBCC_2023-03-26_23_09.h5ad",
# MSSM = "coming soon",
# FULL = "/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/h5ad_final/",
AGING = "/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/h5ad_final/AGING_2023-03-27_02_16.h5ad")
...
ctorder = c('EN_L2_3_IT', 'EN_L3_5_IT_1', 'EN_L3_5_IT_2', 'EN_L3_5_IT_3', 'EN_L5_IT', 'EN_L5_6_NP', 'EN_L6_CT', 'EN_L6_IT', "EN_L6_IT_1", "EN_L6_IT_2", 'EN_NF', 'IN_ADARB2', 'IN_LAMP5', 'IN_PVALB', 'IN_PVALB_CHC', 'IN_SST', 'IN_VIP', 'Oligo', 'OPC', 'Astro', 'Micro_PVM', 'CD8_T', 'PC', 'VLMC','Endo', "Immune")
...
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
    ...
    # Pseudobulk combining all cells
    counts.list = lapply(assayNames(pb), function(id) assay(pb, id))
    sumCounts = Reduce("+", counts.list)
    assay(pb, "bulk") = sumCounts
...
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
    ...
    if( "pool" %in% colnames(colData(pb)) ){
      form = update(form, ~ . + (1|pool))
    }

    # processAssays
    res.proc = processAssays( pb[1:200,], form, BPPARAM = bpparam)
...
form = ~ (1|SubID) + (1|pool) + (1|Sex) + scale(Age) + (1|date) + (1|flowcell)
