...
# bsub -Is -q premium -R span[hosts=1] -R rusage[mem=20000] -W 64:00 -P acc_CommonMind -n 12 bash

# bsub -Is -q premium -R span[hosts=1] -R rusage[mem=10000] -W 36:00 -P acc_CommonMind -n 12 bash
...
for(cluster_id in cluster_id_options ){
  for(sample_id in sample_id_options ){
    ...
    if( sample_id == "Channel"){
      form = ~ (1|SubID) + Sex + scale(Age) + log(n_genes) + (1|prep) 
    }else{      
      form = ~ Sex + scale(Age) + log(n_genes) + (1|prep) 
    }
