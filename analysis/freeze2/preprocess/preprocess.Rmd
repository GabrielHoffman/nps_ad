...
for(cluster_id in c("class", "subclass", "subtype") ){
  ...
  for(sample_id in c("Channel", "SubID") ){
    ...
    # Residuals
    if( sample_id == "SubID" ){
      form = ~ 1 # (1|poolID) + (1|Sex) + scale(Age)
      fit = dreamlet(res.proc, form, 
                      BPPARAM = SnowParam(12) )

      for( CT in assayNames(res.proc) ){
        file = paste0("/sc/arion/projects/psychAD/NPS-AD/freeze2_rc/residuals/", gsub(".h5ad$", "", basename(params$DATASET)), "_residuals_", sample_id, "_", cluster_id, ".tsv")
