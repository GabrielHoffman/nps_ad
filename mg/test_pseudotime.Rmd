...
res = lapply(names(dfList), function(id){
	...
	if( grepl("fresh_mg", id) ){
		form = dpt_pseudotime ~ braak + (1|subtype) + (1|SubID) + age + (1|sex) + PMI + log(n_genes) + (1|predicted_phase)
	}else{
		form = dpt_pseudotime ~ braak + (1|subtype) + (1|SubID) + age + (1|sex) + PMI + (1|poolID) + log(n_genes) + (1|predicted_phase)
	}
...
res = lapply(names(dfList), function(id){
	...
	res = lapply(as.character(unique(data$subtype)), function(st){
		res = lapply(traits, function(trait){
			...
			if( grepl("fresh_mg", id) ){
				form = dpt_pseudotime ~ (1|SubID) + age + (1|sex) + PMI + log(n_genes) + (1|predicted_phase)
			}else{
				form = dpt_pseudotime ~ (1|SubID) + age + (1|sex) + PMI + (1|poolID) + log(n_genes) + (1|predicted_phase)
			}
...
res = lapply(names(dfList), function(id){
	...
	res = lapply(as.character(unique(data$subtype)), function(st){
		res = lapply(traits, function(trait){
			...
			if( grepl("fresh_mg", id) ){
				form = RankNorm(dpt_pseudotime) ~ (1|SubID) + age + (1|sex) + PMI + log(n_genes) + (1|predicted_phase)
			}else{
				form = RankNorm(dpt_pseudotime) ~ (1|SubID) + age + (1|sex) + PMI + (1|poolID) + log(n_genes) + (1|predicted_phase)
			}
...
form = y ~ offset(log(lib.size)) + scale(dpt_pseudotime) + scale(age) + sex + scale(PMI) + scale(log(n_genes)) + (1|SubID) 
...
fit3 = glmm.nb(form, random = ~ 1|SubID, data)
