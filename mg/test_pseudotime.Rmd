...
rmarkdown::render("test_pseudotime.Rmd");
...
files = dir("/Users/gabrielhoffman/Downloads/data-2/", recursive=TRUE, full.names=TRUE)
...
dfList = lapply( names(files)[1:2], function(id){
	...
	df$Donor =  gsub("-[GCTA]+", "", df$barcodekey)
...
res = lapply(names(dfList), function(id){
	...
	form = dpt_pseudotime ~ braak + (1|subtype) + (1|Donor)

	fit = lmer(form, dfList[[id]], REML=FALSE)
