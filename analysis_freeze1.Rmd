...
pca = function(cobj, useWeights=TRUE){
  if(useWeights ){
      res = prcomp( t(with(cobj, E * (weights))), scale.=FALSE, center=FALSE )
  }else{    
      res = prcomp( t(cobj$E), scale.=FALSE )
  }
  res
}

cobj = crumblr(cellCounts(pbObj[,i])[,-c(6,9)])
...
fig = df_pca %>% filter(dx %in% c("AD", "Control")) %>%
      ggplot(aes(PC1, PC2, color=`Glutamatergic neuron`)) + 
        geom_point() +
        theme_classic() +
...
cor(df_pca[,2:3], df_pca[,c( 25:30)], method="sp")


res = prcomp(logit(fractions))
