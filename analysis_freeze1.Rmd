...
Data freeze includes ??? rounds, `r length(table(sceCombine$batch))` batches, `r length(table(sceCombine$SubID))` donors, and `r format(ncol(sceCombine), big.mark=',')` cells passing QC.
...
# Set levels for Dx2
lvls = sort(unique(as.character(colData(pbObj)$dx)))
lvls = lvls[-which(lvls=="Control")]
colData(pbObj)$dx = factor(colData(pbObj)$dx, c("Control", lvls))

saveRDS(pbObj, file="/sc/arion/projects/CommonMind/hoffman/NPS-AD/work/pbObj_f1.RDS" )

pbObj
```

<!---
pbObj = readRDS("/sc/arion/projects/CommonMind/hoffman/NPS-AD/work/pbObj_f1.RDS")
--->
...
```{r plot.sex} 
df_gene = select(org.Hs.eg.db, keys=c("XIST", "UTY"), keytype="SYMBOL", columns="ENSEMBL")
...
fig = ggplot(as.data.frame(df_sex), aes(XIST, UTY, color=Sex)) + geom_point() + theme_classic() + facet_wrap(~cellType)
```

Sex expression based on XIST and UTY
```{r sex.combine}
chromExpr = assay(sceCombine, "X")[df_gene$ENSEMBL,]
rownames(chromExpr)[rownames(chromExpr) == df_gene$ENSEMBL[1]] = df_gene$SYMBOL[1]
rownames(chromExpr)[rownames(chromExpr) == df_gene$ENSEMBL[2]] = df_gene$SYMBOL[2]

# get total reads
totalReads = colSums2(assay(sceCombine, "X"))
df_totalReads = data.table(totalReads)
rownames(df_totalReads) = colnames(sceCombine)

# sum across cells for each Donor
setAutoBlockSize(1e8)
grid = colAutoGrid(chromExpr, ncol=100000)
df = dreamlet:::colsum_fast(assay(sceCombine[1:10,1:103], "X"), sceCombine$Channel[1:10], grid=grid)

# merge expression with metadata
df_sex = merge(colData(sceCombine), t(df), by="row.names")
...
# combine
df_sex = merge(colData(sceCombine)[,"Sex",drop=FALSE], t(chromExpr), by="row.names")

df_tmp = data.table(id = colnames(sceCombine), Sex=colData(sceCombine)[,"Sex"])
df_tmp2 = data.table(id = colnames(chromExpr), t(as.matrix(chromExpr)))

df_sex = merge(colData(sceCombine)[,"Sex",drop=FALSE], t(chromExpr), by="row.names")



df_sex = merge(df_sex, df_totalReads, by.x = "Row.names", by.y="row.names")

df_sex2 = df_sex %>% 
          group_by(Channel) %>%
          summarise(Sex = unique(Sex),
                    UTY = log2(sum(UTY) + 0.25) - log2(sum(totalReads)) + log2(1e6),
                    XIST= log2(sum(XIST) + 0.25) - log2(sum(totalReads)) + log2(1e6))

ggplot(df_sex2, aes(XIST, UTY, color=Sex)) + geom_point() + theme_classic() + scale_color_manual(values=c("red", "blue", "green"))
```


Sex expression based on summing reads from chrX and chrY
```{r sex.combine2}
idx = rowData(sceCombine)$gene_chrom %in% c("X", "Y")
chromExpr = rowsum(assay(sceCombine[idx,], "X"), rowData(sceCombine[idx,])$gene_chrom)

# get total reads
totalReads = colSums2(assay(sceCombine, "X"))
df_totalReads = data.frame(totalReads)
rownames(df_totalReads) = colnames(sceCombine)

# combine
df_sex = merge(colData(sceCombine), t(chromExpr), by="row.names")
df_sex = merge(df_sex, df_totalReads, by.x = "Row.names", by.y="row.names")

df_sex2 = df_sex %>% 
          group_by(Channel) %>%
          summarise(Sex = unique(Sex),
                    Y = log2(sum(Y) + 0.25) - log2(sum(totalReads)) + log2(1e6),
                    X = log2(sum(X) + 0.25) - log2(sum(totalReads)) + log2(1e6))

ggplot(df_sex2, aes(X, Y, color=Sex)) + geom_point() + theme_classic() + scale_color_manual(values=c("red", "blue", "green"))
```
...
```{r write.residuals}
form = ~ Sex + (1|batch) + (1|round_num) + (1|HTO) + (1|cohort) + dx + scale(Age) + scale(PMI)
...
# Add dream here
# deal with NA's here 
form = ~ (1|SubID) + (1|batch) + (1|round_num) + (1|HTO) + (1|Sex) + (1|dx) + (1|cohort) + scale(Age) + scale(PMI)

i = with(colData(pbObj), !is.na(dx) & !is.na(Age) & !is.na(PMI) & !is.na(Sex))
...
form = ~ (1|SubID) + (1|batch) + (1|round_num) + (1|HTO) + (1|Sex) + (1|cohort) + dx + scale(Age) + scale(PMI)
...
# Check ploidy
```{r ploidy, eval=FALSE}
sce = sceCombine[,1:100000]

# sum reads across chromosome to identify chromsome loss
chromExpr = lapply( levels(rowData(sce)$gene_chrom), function(chrom){
