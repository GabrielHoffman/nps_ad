...
# cells based on Dx
table(sceCombine$Dx_AD)

# AD cases and controls
tab = with(colData(sceCombine), unique(data.frame(SubID, Dx_AD, Sex, Age)))
table(tab$Dx_AD)
table(tab$Sex)
hist(tab$Age)
...
sce = sceCombine[,!is.na(sceCombine$Dx_AD)]
...
```{r write2}
sce$static = c()
# sce = sce[,sce$SubID %in% df$Row.names[!drop.ids]]
sce = sce[,sce$SubID %in% df$Row.names]
...
#' 
#' Identify multivariante outliers using Mahalanobis distance using mean and covariance estimated either with standard or robust methods.
#' 
#' @param data matrix of data 
#' @param robust use robust methods
#'
#' @details
# The distance follow a chisq distrubtion under the null with standard methods.  So use \code{qchisq(p = 0.999 , df = 2)} to get cutoff to keep 99.9% of samples under the null for data with 2 columns
#'
outlier = function(data, robust = TRUE){

  if( robust ){
    res = MASS::cov.rob(data)
...
df = reducedDim(sce, "X_umap") %>%
    data.frame(ID = colnames(sce), CellType = sce$subclass) %>%
    group_by(CellType) %>%
    summarize( ID, d = outlier(data.frame(X1, X2)))   

id.outlier = df$ID[df$d > 12]

idx = match(id.outlier, colnames(sce))
...
outprefix = "/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Jan_25_2023"
...
echo $(seq 1 15 | parallel -P1 ls /sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Jan_25_2023_chunk{}.h5ad) | tr ' ' '\n' > list.txt

cd /hpc/users/hoffmg01/.cache/R/basilisk/1.8.0/0/pkgs/
OUTFILE=/sc/arion/projects/psychAD/NPS-AD/public_release_0/PsychAD_r0_Jan_25_2023.h5ad 
