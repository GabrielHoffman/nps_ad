...
```{r analysis, fig.height=7, fig.width=7}
library(mashr)
...
heatmap.2(C, symm=TRUE, trace = "none", density.info = "none", col = bluered(100), symbreaks=TRUE, cellnote=round(C, 2), notecol="black", main="Original", key.xlab="Correlation", key.title='', keysize=1)
```

Spearman correlation between posterior beta values
```{r beta.post, fig.height=8, fig.width=8}
C = cor(beta.post, use='pairwise.complete.obs', method="sp")

par(mar=c(7,4,4,2)+0.1) 
heatmap.2(C, symm=TRUE, trace = "none", density.info = "none", col = bluered(100), symbreaks=TRUE, cellnote=round(C, 2), notecol="black", main="mashr", key.xlab="Correlation", key.title='', keysize=1, cexCol=1,margins=c(12,8))
```
...
```{r sign.share, fig.height=9, fig.width=9}
# sharing based on sigh
C = get_pairwise_sharing(m2, factor=0)

heatmap.2(C, symm=TRUE, trace = "none", density.info = "none", col = bluered(100), symbreaks=TRUE, cellnote=round(C, 2), notecol="black", main="Sign sharing", key.xlab="Correlation", key.title='', keysize=1)
```
